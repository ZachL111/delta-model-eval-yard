...
`delta-model-eval-yard` is centered on ml utilities work in Haskell. The technical target is: Create a Haskell reference implementation for eval workflows, centered on format conversion, round-trip fixtures, and lossless normalization checks.
...
The score is deterministic. The current threshold is 169. Risk has a penalty of 7, latency has a penalty of 3, and weight contributes a bonus of 5.
